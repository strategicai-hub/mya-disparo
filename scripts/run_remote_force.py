#!/usr/bin/env python3
"""Roda scripts/force_response.py dentro do container mya-disparo_api via Portainer exec."""
import os
import sys
import requests
import urllib3
...
def main():
    ...
    with open(os.path.join(here, "force_response.py"), "r", encoding="utf-8") as f:
        script_src = f.read()

    container_id = find_container_id()
    print(f"[1/2] Container: {container_id[:12]}")
    print(f"[2/2] Executando force_response.py...\n")
    out = exec_command(container_id, ["python", "-c", script_src])
    print(out)
